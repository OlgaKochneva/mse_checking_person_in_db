...
if __name__ == '__main__':
    os.chdir('..')
    path_to_imgs_people = '../resources/resources_for_test_generator/img_people'
    bg = cv2.imread('../resources/resources_for_test_generator/bgs/2.jpg')
    f = open('video_testing_dataset.csv', 'w', newline='')
    img_human_scale_factor_range = 0.7, 1
    black_color_threshold = 2
    frame_count = 100
    fps = 2

    video_writer = cv2.VideoWriter(f'{os.path.abspath("video_testing.avi")}',
